...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $chooseBtn = $('' +
         ...
         '<button class="ssi-button success" type="button">' + this.language.chooseFiles + '</button>' +
         '</span>').append(element);
        var $uploadBtn = $('<button id="ssi-uploadBtn" type="button" class="ssi-button success ssi-hidden" >' +
         '<span class="ssi-btnIn">' + this.language.upload + ' </span>' +
         '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
        var $clearBtn = $('<button id="ssi-clearBtn" type="button" class="ssi-hidden ssi-button info" >' + this.language.clear +
         '</button>');
        var $abortBtn = $('<button id="ssi-abortBtn" type="button" class="ssi-button error ssi-cancelAll ssi-hidden" ><span class="inBtn">' + this.language.abort + ' </span></button>');
...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.toUploadFiles = function (files) {
		...
		if ($.inArray(ext, this.options.allowed) === -1 && $.inArray('any', this.options.allowed) === -1) { // if requested file not allowed
